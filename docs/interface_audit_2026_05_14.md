# Interface Audit 2026-05-18

This note records the current v7-only interface check across frontend,
GNN training, labeling, inference, and generation. It supersedes the original
2026-05-14 audit while keeping the filename for compatibility.
...
full MinerU v7 styled JSON
  -> GNNViewAdapter
     - metadata/noise/annotation excluded
     - figure/table/algorithm converted to float proxies
  -> graph.pt
...
| label generation | `scripts/pipeline/relabel_manifest.py` / `src/reasoning/label_generator.py` | Uses `GNNViewAdapter`, so graph nodes and labels share the same graph-visible node view and float-proxy policy. |
| training | `scripts/pipeline/train_edge_gnn_full.py` | Requires manifest/root explicitly. Locked baseline: `v7_registry_adapteraware_20260515_181724_labeled.json`; active experiment: `v7_floatproxy_adapter_20260516_205926_labeled.json` after relabel completes. |
| ablation | `configs/ablation_matrix_v7_adapteraware_20260514_2109.json` | Maintained matrix filename is historical; callers must pass the intended manifest/graph root. Older v1/v2 are historical. |
...
- `TreeDecoder.render_document` remains for tests and explicit
  old tree renderer regression debugging.
...
The locked baseline adapter-aware run started from existing v7 JSON only and is
kept for reproducible comparison:
...
OUTPUT_MANIFEST=data/00_manifests/v7_registry_adapteraware_20260515_181724_labeled.json
EDGE_ATTR_DIM=22
```

MinerU is not rerun in this path.

The current float-proxy experiment also starts from existing v7 JSON only:
...
This run creates a new graph/label family and does not overwrite the locked
baseline manifests or checkpoints.
