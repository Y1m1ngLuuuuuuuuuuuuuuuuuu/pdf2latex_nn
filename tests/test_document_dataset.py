...
def make_graph(torch, Data, *, edge_index=None, x=None, edge_attr=None):
    ...
    data = Data(
        x=torch.zeros((2, 813), dtype=torch.float64) if x is None else x,
...
def test_sanitize_graph_data_casts_float32_and_clamps_non_finite_values():
    ...
    data = make_graph(
        torch,
        Data,
        x=torch.full((2, 813), float("nan"), dtype=torch.float64),
...
def test_document_dataset_skips_graph_when_alignment_quality_is_too_low(tmp_path):
    ...
    data = Data(x=torch.zeros((3, 813)), edge_index=edge_index, edge_attr=torch.zeros((3, 11)))
