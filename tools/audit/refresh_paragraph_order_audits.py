...
output, and writes a compact summary. It reports legacy/body metrics alongside
visible-prose order metrics. It does not regenerate LaTeX, rerun MinerU, train,
or delete prior artifacts.
"""
...
def main() -> int:
    ...
    (args.output_dir / "refresh_manifest.json").write_text(
        json.dumps({"schema_version": "paragraph_order_audit_refresh_v2", "tasks": [task.__dict__ for task in tasks]}, indent=2)
        + "\n",
        encoding="utf-8",
    )
...
def run_task(task: AuditTask, include_list_items: bool, max_examples: int, env: dict[str, str]) -> dict[str, Any]:
    ...
    return {
        **task.__dict__,
        ...
        "visible_prose_source_coverage_rate": summary.get("visible_prose_source_coverage_rate"),
        "visible_prose_ordered_coverage_rate": summary.get("visible_prose_ordered_coverage_rate"),
        "visible_prose_order_inversion_rate": summary.get("visible_prose_order_inversion_rate"),
        "adjacent_prose_inversion_rate": summary.get("adjacent_prose_inversion_rate"),
        "displaced_prose_paragraph_rate_010": summary.get("displaced_prose_paragraph_rate_010"),
        "displaced_prose_paragraph_rate_015": summary.get("displaced_prose_paragraph_rate_015"),
        "visible_prose_lis_disorder_rate": summary.get("visible_prose_lis_disorder_rate"),
...
def write_report(path: Path, rows: list[dict[str, Any]], errors: list[dict[str, Any]], workers: int) -> None:
    ...
    lines = [
        ...
        "",
        "| root | group | variant | docs | body cov | ordered body cov | body inv | visible prose ordered cov | visible prose inv | adjacent inv | displaced >.10 | LIS disorder | missing | wrong |",
        "| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for (root, group, variant), group_rows in sorted(grouped.items()):
        lines.append(
            "| {root} | {group} | {variant} | {docs} | {body_cov} | {ordered_cov} | {inv} | {visible_ordered} | {visible_inv} | {adjacent_inv} | {displaced} | {lis} | {missing} | {wrong} |".format(
                root=f"`{root}`",
                group=f"`{group}`",
                variant=f"`{variant}`",
                docs=len(group_rows),
                ...
                visible_ordered=fmt(mean(row.get("visible_prose_ordered_coverage_rate") for row in group_rows)),
                visible_inv=fmt(mean(row.get("visible_prose_order_inversion_rate") for row in group_rows)),
                adjacent_inv=fmt(mean(row.get("adjacent_prose_inversion_rate") for row in group_rows)),
                displaced=fmt(mean(row.get("displaced_prose_paragraph_rate_010") for row in group_rows)),
                lis=fmt(mean(row.get("visible_prose_lis_disorder_rate") for row in group_rows)),
